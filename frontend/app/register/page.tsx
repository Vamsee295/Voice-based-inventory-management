    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F7F7F4] p-4 font-sans">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-[#E5E5E0] p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[12px] text-[#5F6673] mt-1 uppercase tracking-wider font-bold">Workspace Setup</p>
        </div>

        <h2 className="text-[18px] font-bold text-[#111318] mb-6 text-center">Create a New Workspace</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[12px] font-bold text-[#111318] mb-1">Business Name</label>
            <input
              type="text"
              name="businessName"
              required
          ...
          <div>
            <label className="block text-[12px] font-bold text-[#111318] mb-1">Your Name</label>
            <input
              type="text"
              name="name"
              required
          ...
          >
            {loading ? <span className="material-symbols-outlined animate-spin text-[18px]">sync</span> : 'Create Workspace'}
          </button>
        </form>

        <div className="mt-6 text-center text-[13px] text-[#5F6673]">
          Already have an account? <Link href="/login" className="text-[#2457FF] font-bold hover:underline">Sign In</Link>
        </div>
      </div>
    </div>
  );
